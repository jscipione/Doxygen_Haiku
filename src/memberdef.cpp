                                 ...
                                 const QCString & /*scopeName*/,MemberDef *md)
{
  ...
  while (a)
  {
    ...
    if (!a->name.isEmpty() || (a->name.isEmpty() && a->type=="...")) // argument has a name
    {
      ...
      if (a->name.isEmpty()) ol.docify(a->type);
      else ol.docify(a->name);
                         ...
                         )
{
  ...
  {
    // write the overloaded method or function name
    if (overloaded && (isMethod() || isFunction()))
    {
